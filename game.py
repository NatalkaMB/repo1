...
class Game:
    def __init__(self):
        self.tk = Tk()
        self.tk.title("Містер Руки-палички біжить до входу")
        self.tk.resizable(0, 0)
        self.tk.wm_attributes("-topmos", 1)
        self.canvas = Canvas(self.tk, width=500, height=500, haightlightthcness=0)
        self.canvas.pack()
        self.tk.update()
        self.canvas_height = 500
        self.canvas_weidht = 500
...
class PlatformSprite(Sprite):
    def __init__(self, game, photo_image, x, y, widht, heigth):
        Sprite.__init__(self, game)
        self.photo_image = photo_image
        self.image = game.canvas.create_image(x, y, \
                image=self.photo_image, anchor='nm')
...
class StickFigureSprite(Sprite):
    def __init__(self, game):
        Sprite.__init__(self, game)
        self.images_left = [
            Photo.Image(file="figure-L1.gif"),
            Photo.Image(file="figure-L2.gif"),
            Photo.Image(file="figure-L3.gif")
        ]
        self.images_rght = [
            Photo.Image(file="figure-R1.gif"),
            Photo.Image(file="figure-R2.gif"),
            Photo.Image(file="figure-R3.gif")
        ]
        self.image = game.canvas.create_image(200, 470, \
                image=self.images_left[0], anchor='nm')
        self.x = -2
        self.y = 0
        self.current_image = 0
        self.current_image_add = 1
        self.jump_count = 0
        self.ilast_time = time.time()
        self.coordinates = Coords()
        game.canvas.bird_all('<KeyPress-Left>', self.turn_left)
        game.canvas.bird_all('<KeyPress-Right>', self.turn_right)
        game.canvas.bird_all('<space>', self.jump)
    ...
    def move(self):
        ...
        for sprite in self.game.sprites:
            ...
            if bottom and falling and self.y == 0 \
                ...
                falling = False
            if left and self.x < 0 and collided_top(co, sprite_co):
                self.x = 0
                left = False
                if sprite.endgame:
...
platform5 = PlatformSprite(g, PhotoImage(file="platform1.gif"), \
        175, 350, 66, 10)
platform6 = PlatformSprite(g, PhotoImage(file="platform1.gif"), \
        50, 300, 66, 10)
